...
export default function ReciboImpresionModal({
  ...
  // ← ← ← FUNCIÓN DE IMPRESIÓN OPTIMIZADA PARA TÉRMICA 80MM ← ← ←
  const handlePrint = () => {
    const printContent = document.getElementById('recibo-para-imprimir');
    if (!printContent || !recibo) return;
        ...
        <style>
          * { margin: 0; padding: 0; box-sizing: border-box; }
          body { 
            ...
            font-size: 12px; 
            color: #000;
            width: 72mm; /* Ancho útil de papel 80mm */
            margin: 0 auto;
            padding: 5mm;
            background: #fff;
          }
      ...
      <body>
        ...
        <div class="info">
          ...
          <div class="info-row">
            ...
            <span>${recibo.session_caja_turno}</span>
          </div>` : ''}
        </div>
        ...
        <div class="totals">
          ...
          <div class="total-row">
            <span>Descuento:</span>
            <span>-${formatMoney(recibo.descuento)}</span>
          </div>` : ''}
          ${recibo.propina_total && parseFloat(recibo.propina_total) > 0 ? `
          <div class="total-row">
            <span>Propina:</span>
            <span>+${formatMoney(recibo.propina_total)}</span>
          </div>` : ''}
        ...
        ${recibo.notas ? `
        <div style="margin-top:8px; padding:6px; background:#f8f9fa; border:1px solid #e9ecef; font-size:10px;">
        ...
        <div class="footer">
          ...
          <p style="font-size:9px; color:#666; margin-top:4px;">
            Impreso: ${new Date().toLocaleString('es-CO', { timeZone: 'America/Bogota', day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit' })}
          </p>
        </div>
      </body>
      </html>
