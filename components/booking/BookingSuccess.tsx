...
async function loadData() {
  try {
   ...
   // ← NUEVO: Cargar datos del salón desde configuración (para mostrar en UI)
    async function cargarConfiguracionSalon() {
      try {
        ...
        if (configRes.ok) {
          ...
          // ← NUEVO: Dirección del salón
          if (config?.direccion) {
            setDireccionSalon(config.direccion);
            console.log('✅ Dirección cargada:', config.direccion);
          }
          
          // ← NUEVO: Horario del salón
          if (config?.horario_atencion) {
            setHorarioSalon(config.horario_atencion);
            console.log('✅ Horario cargado:', config.horario_atencion);
          }
        }
      } catch (err) {
        console.warn('⚠️ No se pudo cargar configuración del salón');
      }
    }
    
    await cargarConfiguracionSalon();;
  ...
  return (
    <div className="text-center py-12">
      ...
      <div className="mt-12 text-sm text-gray-500 space-y-2">
        <p>📍 <strong>Dirección:</strong> {direccionSalon || 'Consultar en sitio web'}</p>
        <p>🕐 <strong>Horario:</strong> {horarioSalon || 'Lunes-Viernes 9AM-7PM'}</p>
      </div>
    </div>
  );
}